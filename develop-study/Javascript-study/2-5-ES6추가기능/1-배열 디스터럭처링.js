...
const userNames = [`김철수`, `홍길동`, `고구마`, `바나나`, '아이스크림'];
//  배열 내 데이터를 다시 각각의 변수로 저장할 시
const kim = userNames[0]; // 김철수
const hong = userNames[1]; // 홍길동
const go = userNames[2]; // 고구마
const ba = userNames[3]; // 바나나
const ice = userNames[4]; // 아이스크림

console.log(`kim: ${kim}, hong: ${hong}, go: ${go}`);

// 간략화
const [kim1, hong1, ...others] = userNames;

console.log(`kim: ${kim1}, hong: ${hong1}, 나머지: ${others}`);
...
[first, second] = [second, first]; // 요약
// const two = [second, first]; // 1단계. 값을 바꾼 배열을 생성
// // console.log(two); // 
// const [a, b] = two; // 2단계. 값을 바꾼 배열을 기존 배열에 대입
